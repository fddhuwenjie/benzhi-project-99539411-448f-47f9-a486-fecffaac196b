...
func (s *Store) Commit(snap *Snapshot, event domain.AuditEvent) error {
	...
	line = append(line, '\n')
	auditFile, err := os.OpenFile(s.auditPath(snap.Batch.BatchID), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o640)
	if err != nil {
		return err
	}
	...
	rollback := func() { _ = auditFile.Truncate(pos); _ = auditFile.Sync(); _ = auditFile.Close() }
	...
	s.invalidateSnapshot(snap.Batch.BatchID)
