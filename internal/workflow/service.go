...
func (s *Service) Get(id string) (*domain.DendroBatch, error) {
	...
	if err != nil {
		return nil, contextualRepositoryError("读取批次快照", err)
	}
...
func (s *Service) Manifest(id string) (*domain.Manifest, error) {
	...
	if err != nil {
		return nil, contextualRepositoryError("读取封存清单", err)
	}
	return m, nil
}
func (s *Service) Events(id string) ([]domain.AuditEvent, error) {
	...
	if err != nil {
		return nil, contextualRepositoryError("读取审计事件", err)
	}
	return e, nil
}
...
func contextualRepositoryError(operation string, err error) error {
	translated := translateRepo(err)
