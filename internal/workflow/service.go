...
func New(store *repository.Store) *Service {
	return &Service{store: store, locks: newKeyedLocks(), clock: func() time.Time { return time.Now().UTC() }}
}
...
func (s *Service) mutate(batchID, action string, meta CommandMeta, status int, fn mutation) (Result, error) {
	return s.mutateContext(context.Background(), batchID, action, meta, status, fn)
}

func (s *Service) mutateContext(ctx context.Context, batchID, action string, meta CommandMeta, status int, fn mutation) (Result, error) {
	if err := validateMeta(meta); err != nil {
		return Result{}, err
	}
	unlock, err := s.locks.lockContext(ctx, batchID)
	if err != nil {
		return Result{}, err
	}
	defer unlock()
