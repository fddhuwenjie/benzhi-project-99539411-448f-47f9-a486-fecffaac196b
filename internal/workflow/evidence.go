...
func (s *Service) RegisterImages(batchID string, cmd RegisterImagesCommand) (Result, error) {
	return s.RegisterImagesContext(context.Background(), batchID, cmd)
}

func (s *Service) RegisterImagesContext(ctx context.Context, batchID string, cmd RegisterImagesCommand) (Result, error) {
	return s.mutateContext(ctx, batchID, "REGISTER_IMAGES", cmd.CommandMeta, 200, func(batch *domain.DendroBatch, _ *repository.Snapshot, now time.Time) (*CommandResponse, string, string, error) {
